...
func (f *FileLogger) initFile() {
	...
	if err != nil {
		panic(fmt.Errorf("open log file %s failed, %v", logName, err))
	}
	f.file = fileObj
	...
	if err != nil {
		panic(fmt.Errorf("open log file %s failed, %v", errLogName, err))
	}
...
func (f *FileLogger) log(level Level, format string, args ...interface{}) {
	...
	// 如果是error或者fatle级别的日志还要记录到 f.errFile
	if level >= ErrorLevel {
